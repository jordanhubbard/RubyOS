...
target_build="$root/build/baremetal/ruby-freestanding-x86_64"
...
make -j"$jobs" hardenflags= optflags=-O2 \
    XCFLAGS='-fno-strict-overflow -fvisibility=hidden -fexcess-precision=standard -DRUBY_EXPORT -fno-pie -mno-red-zone $(INCFLAGS)' \
    libruby-static.a
test -s libruby-static.a
touch .rubyos-built
echo "freestanding x86_64 CRuby archive: $target_build/libruby-static.a"
