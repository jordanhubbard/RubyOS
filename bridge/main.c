/*
 * rubyos_bridge — host-side companion that serves draw/audio/input
 * ops to RubyOS over a JSON-RPC byte stream.
 *
 * Forked from PythonOS's protocol-v1 companion; see PYTHONOS-LICENSE.
 *
...
static void bridge_log(const char *level, const char *fmt, ...) {
    fprintf(stderr, "[rubyos_bridge:%s] ", level);
    va_list ap;
    va_start(ap, fmt);
...
static uint64_t slow_threshold_us(void) {
    const char *value = getenv("RUBYOS_BRIDGE_SLOW_US");
    if (!value) value = getenv("PYTHONOS_BRIDGE_SLOW_US");
...
static int op_hello(BridgeState *st, int id, cJSON *params) {
    ...
    cJSON_AddStringToObject(r, "agent",      "rubyos_bridge");
...
static int op_display_open(BridgeState *st, int id, cJSON *params) {
    ...
    const char *title = cJSON_IsString(jt) ? jt->valuestring : "RubyOS";
    const char *desktop_mode = getenv("RUBYOS_DESKTOP_MODE");
    if (!desktop_mode) desktop_mode = getenv("PYTHONOS_DESKTOP_MODE");
...
static const char *export_directory(void) {
    ...
    const char *configured = getenv("RUBYOS_EXPORT_DIR");
    if (!configured) configured = getenv("PYTHONOS_EXPORT_DIR");
...
static int op_host_export_finish(BridgeState *st, int id, cJSON *params) {
    ...
    cJSON *r = cJSON_CreateObject();
    cJSON_AddStringToObject(r, "path", slot->final_path);
    LOG_INFO("exported RubyOS file to %s", slot->final_path);
    slot->token = 0;
    return send_ok(st->fd, id, r);
}
 ...
 * Adding a new function = one entry. RubyOS doesn't carry a custom
...
static int socket_buffer_bytes(void) {
    const char *value = getenv("RUBYOS_BRIDGE_SOCKET_BUFFER");
    if (!value) value = getenv("PYTHONOS_BRIDGE_SOCKET_BUFFER");
    if (!value || !value[0]) return 4 * 1024 * 1024;
    char *end = NULL;
    long parsed = strtol(value, &end, 10);
    if (end == value || *end != '\0' || parsed < 65536 ||
        parsed > 64 * 1024 * 1024) {
        LOG_WARN("ignoring invalid RUBYOS_BRIDGE_SOCKET_BUFFER=%s", value);
        return 4 * 1024 * 1024;
    }
    return (int)parsed;
}
...
static int run_selftest(void) {
    ...
    SDL_Window *w = SDL_CreateWindow("rubyos_bridge selftest",
                                     SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
...
static void usage(FILE *out, const char *argv0) {
    fprintf(out,
        ...
        "  --connect-tcp HOST:PORT connect to a RubyOS TCP bridge listener\n"
